...
def compute_average_runtimes(runtimes):
    groups = {}
    for density, size, _, runtime in runtimes:
        key = (density, size)
    ...
    return [
        (density, size, round(sum(stats) / len(stats), 3))
        for (density, size), stats in groups.items()
    ]
...
def measure_runtime(
    ...
    try:
        for density in densities:
            for size in sizes:
                ...
                print(f"Running with size {size}, and density {density}")
                for iteration in range(10):
                    ...
                    E = sum([len(e) for e in graph.values()])
                    runtimes.append((density, size, E, runtime))
    ...
    print_markdown_table(ave_runtimes, ["Density", "Size ", "Time (sec)"])
